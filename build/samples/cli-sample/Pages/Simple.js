import React, { useEffect } from "react";
import { ScrollingBox, progressEmitter, } from "../dependencies/ink-components.js";
export function SimpleLog() {
    const [log, setLog] = React.useState(["nothing to show"]);
    useEffect(() => {
        const logListener = (path, message) => {
            setLog((prevLog) => [...prevLog, `L[${path}] ${message}`]);
        };
        const updateListener = (path, value) => {
            setLog((prevLog) => [
                ...prevLog,
                `S[${path}] ${value.state}: ${value.status} (${value.details})`,
            ]);
        };
        progressEmitter.on("log", logListener);
        progressEmitter.on("update", updateListener);
        return () => {
            progressEmitter.off("log", logListener);
            progressEmitter.off("update", updateListener);
        };
    }, []);
    return React.createElement(ScrollingBox, { data: log });
}
//# sourceMappingURL=Simple.js.map