...
export const AnimatedProgressBar = (props) => {
    ...
    const { filledChar, emptyChar, waveChar, animationStyle, animationSpeed, showPercentage, colors } = mergedStyle;
    ...
    return (React.createElement(Box, null,
        React.createElement(Text, { color: colors.filled }, progressChars.slice(0, Math.round((value / 100) * width)).join('')),
        React.createElement(Text, { color: colors.empty }, progressChars.slice(Math.round((value / 100) * width)).join('')),
        showPercentage && (React.createElement(Text, { color: colors.text },
            " ",
            percentage,
            "%"))));
};
